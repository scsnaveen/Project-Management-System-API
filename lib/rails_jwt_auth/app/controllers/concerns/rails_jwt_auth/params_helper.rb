  ...
  module ParamsHelper
    ...
    def registration_create_params
      params.require(RailsJwtAuth.model_name.underscore).permit(
        RailsJwtAuth.auth_field_name, :password, :password_confirmation,:name,:organization_id,:otp_secret,:enabled_check
      )
    end
    ...
    def invitation_create_params
      params.require(:invitation).permit(RailsJwtAuth.email_field_name,:invitation_by)
    end
    ...
    # end
     def profile_update_params
      params.permit(:name, :phonenumber, :username, :picture, :password)
    end
